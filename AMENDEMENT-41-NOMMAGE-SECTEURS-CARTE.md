...
- **Backend (V1)** : pré-calcul par secteur (`owner_crew_id`, `owner_percent`, `top_rival_crew_id`, `top_rival_percent`, `neutral_percent`, `status`, `pressure_score`, `name`…) → le client reçoit tuiles + secteurs agrégés + statuts, jamais toute la base.
