...
**Ratifiée par l'ADR-012** (`docs/DECISIONS.md`), décision du fondateur du 9 septembre 2026, mot pour mot : « Le cahier de septembre remplace le MASTER. » D-20 avait été écrite AVANT cette décision et sans connaître `GRYD_MASTER_PROMPT.md`, `docs/DECISIONS.md` ni `CLAUDE.md` ; l'ADR-012 la confirme et consigne les tensions ouvertes avec ADR-002 (reset), ADR-003 (seuils), ADR-006 (Rouen), ADR-008 (palette), ADR-010 (H3) et ADR-011 (gratuit) — elles se tranchent là, par un ADR daté. L'ordre d'autorité est désormais le même dans `CLAUDE.md` et `AGENTS.md` : 0 cahier · 1 ADR · 2 MASTER et SPEC-* archivés · 3 le reste.
...
| 1 | [docs/DECISIONS.md](docs/DECISIONS.md) | ADR datés. L'ADR-012 ratifie D-20 et consigne les tensions ouvertes ; toute contradiction entre le cahier et une décision antérieure se tranche par un nouvel ADR, jamais par une ligne d'`AGENTS.md`. |
| 2 | [AGENTS.md](AGENTS.md) et [CLAUDE.md](CLAUDE.md) | Instructions d'exécution du dépôt — la même constitution pour Codex et pour Claude ; leur rang 0 désigne le cahier ci-dessus. Les invariants de sécurité, d'autorité serveur, de données réelles et d'honnêteté restent applicables. |
| 3 | [Règles partagées](packages/shared/src/game-rules.ts), [tokens](packages/shared/src/design-tokens.ts), migrations et code | Contrats d'implémentation à comparer au cahier. Une constante ou une fonction existante ne peut pas modifier une exigence produit. Les copies sous `supabase/functions/_shared/` sont générées. |
| 4 | [Recette de septembre](docs/product/REFONTE_2026_RECETTE.md) | État observé du travail livré, validations exécutées et limites restantes. Ce document ne crée aucune nouvelle règle produit et ne certifie aucun déploiement. |
