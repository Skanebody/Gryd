...
export function useCommercialCollections2026() {
  ...
  async function action(id: CommercialCollectionId2026 | 'restore') {
    if (!ownerId || !current() || lock.current || !supabase || !purchasesCapability().available) return 'unavailable' as const;
    const product = id === 'restore' ? null : productFor(id);
    if (id!=='restore' && (!product || own?.rows?.find(r=>r.id===id)?.owned)) return 'unavailable' as const;
    ...
    try {
      ...
      if(result.kind==='cancelled'||result.kind==='failed') return result.kind;
      const auth=await supabase.auth.getSession();
      if(!current() || auth.data.session?.user.id!==ownerId) return 'cancelled' as const;
      ...
      if(!current()) return 'cancelled' as const;
      reload();
      return rows && (id==='restore'?rows.some(r=>r.owned):rows.some(r=>r.id===id&&r.owned)) ? 'confirmed' as const : 'pending' as const;
