...
import { CREW_SWITCH_COOLDOWN_DAYS, EVENTS, fonts, refonteColors as c, type CrewRecruitmentStatus } from '@klaim/shared';
...
import { isCrewRole, leaveVerdict } from '../crew/memberRoles';
import { useRealCrew, randomCrewColor, type CrewRefusal, type RealCrewMember } from '../crew/real';
...
function CrewHomeContents() {
  ...
  const [access, setAccess] = useState<CrewRecruitmentStatus>('invite_only');
  ...
  const refusal = (reason: CrewRefusal): string => {
    ...
    if (reason === 'name_unavailable') return copy('Ce nom est déjà utilisé. Choisis-en un autre.', 'This name is already in use. Choose another.');
    ...
    if (reason === 'unsupported_server') return copy('La création de crew est momentanément indisponible.', 'Crew creation is temporarily unavailable.');
  ...
  async function createCrew() {
    if (busy || !cityId || !name.trim()) return;
    setBusy(true); setError(null);
    const result = await crew.createCrew(name, randomCrewColor(), cityId, access);
    setBusy(false);
    if (result.ok) { track(EVENTS.crewCreated); setMode('invite'); crew.reload(); }
  ...
  async function joinCrew() {
    ...
    setBusy(false);
    if (result.ok) { track(EVENTS.crewJoined, { via: 'code' }); setMode('home'); crew.reload(); }
    else setError(refusal(result.reason));
  }
  async function leaveCrew() {
    ...
    setBusy(false);
    if (result.ok) { setConfirmLeave(false); crew.reload(); }
    else setError(refusal(result.reason));
  }
  ...
  return <>
    <ProfilePage tone="light" title="Crew" right={crew.crew ? <Pressable accessibilityRole="button" accessibilityLabel={copy('Gérer le crew', 'Manage crew')} onPress={() => router.push('/crew-edit')} style={local.headerAction}><GrydIcon name="settings" size={20} color={c.ink} /></Pressable> : undefined}>
      {mode === 'create-name' || mode === 'create-access' || mode === 'join' ? <View style={local.form}>
        <Pressable accessibilityRole="button" onPress={() => goMode(mode === 'create-access' ? 'create-name' : 'home')} style={local.back}><GrydIcon name="chevronLeft" size={20} color={c.ink} /><Text style={local.actionText}>{copy('Retour', 'Back')}</Text></Pressable>
        <Text style={local.kicker}>{mode === 'join' ? copy('INVITATION', 'INVITATION') : mode === 'create-name' ? copy('01 / IDENTITÉ', '01 / IDENTITY') : copy('02 / ACCÈS', '02 / ACCESS')}</Text>
        <Text style={local.homeTitle}>{mode === 'join' ? copy('Rejoindre un crew', 'Join a crew') : mode === 'create-name' ? copy('Créer un crew', 'Create a crew') : copy('Qui peut rejoindre ?', 'Who can join?')}</Text>
        {mode === 'create-name' ? <>
          <Text style={local.body}>{copy('Choisis un nom et une ville pour ton groupe.', 'Choose a name and city for your group.')}</Text>
          <Text style={local.rowTitle}>{copy('Nom du crew', 'Crew name')}</Text>
          <TextInput accessibilityLabel={copy('Nom du crew', 'Crew name')} value={name} onChangeText={setName} placeholder={copy('Le nom de votre groupe', 'Your group name')} placeholderTextColor={c.muted} style={local.input} autoCapitalize="words" />
          <CityField selectedId={cityId} onSelect={city => setCityId(city.status === 'open' ? city.cityId : null)} openOnly />
          <View style={local.compact}><ProfileButton tone="light" label={copy('Continuer', 'Continue')} disabled={!name.trim() || !cityId} onPress={() => goMode('create-access')} /></View>
        </> : mode === 'create-access' ? <>
          <Text style={local.body}>{name}</Text>
          {([{ key: 'invite_only', title: copy('Privé, sur invitation', 'Private, by invitation'), detail: copy('Un code pour les personnes que tu choisis.', 'A code for the people you choose.') },
            { key: 'on_request', title: copy('Sur demande', 'By request'), detail: copy('Vous choisissez qui rejoint le groupe.', 'You choose who joins the group.') },
            { key: 'open', title: copy('Ouvert à tous', 'Open to everyone'), detail: copy('Les sportifs de votre ville peuvent vous rejoindre.', 'Local athletes can join your group.') }] as const).map(option =>
            <Pressable key={option.key} accessibilityRole="radio" accessibilityState={{ selected: access === option.key }} aria-checked={access === option.key} onPress={() => setAccess(option.key)} style={[local.access, access === option.key && local.accessSelected]}>
              <View style={local.flex}><Text style={local.rowTitle}>{option.title}</Text><Text style={local.copy}>{option.detail}</Text></View><GrydIcon name={access === option.key ? 'check' : 'plus'} size={20} color={c.ink} />
            </Pressable>)}
          <Text style={local.copy}>{copy('Respecter chacun, accueillir les nouveaux et garder les lieux privés hors du fil.', 'Respect everyone, welcome newcomers and keep private places out of the feed.')}</Text>
          <View style={local.compact}><ProfileButton tone="light" label={copy('Créer le crew', 'Create crew')} onPress={() => void createCrew()} busy={busy} /></View>
        </> : <>
      ...
      !crew.crew ? <View style={local.layout}>
        <View style={local.hero}>
          ...
          <View style={local.heroContent}><Text style={local.heroTitle}>{copy('Rejoindre un crew', 'Join a crew')}</Text><Text style={local.heroCopy}>{copy('Des rendez-vous, un groupe et des sorties à partager.', 'Meetups, a group and activities to share.')}</Text>
            {session && crew.ready ? <Pressable accessibilityRole="button" onPress={() => goMode('create-name')} style={local.heroAction}><Text style={local.heroActionText}>{copy('Créer mon crew', 'Create my crew')}</Text><View style={local.primaryCircle}><GrydIcon name="plus" size={20} color={c.ink} /></View></Pressable>
              : configured ? <Pressable accessibilityRole="button" onPress={() => router.push('/sign-in')} style={local.heroAction}><Text style={local.heroActionText}>{copy('Me connecter', 'Sign in')}</Text><View style={local.primaryCircle}><GrydIcon name="arrowUpRight" size={20} color={c.ink} /></View></Pressable>
        ...
        {session && crew.ready ? <View style={local.bento}>
          <CrewTile icon="link" title={copy('Avec un code', 'With a code')} detail={copy('Tu as reçu une invitation.', 'You received an invitation.')} onPress={() => goMode('join')} prominent />
          <CrewTile icon="location" title={copy('À proximité', 'Nearby')} detail={copy('Les crews de ta ville.', 'Crews in your city.')} onPress={() => router.push('/crew-discovery')} />
        </View> : <View style={local.guestNote}><GrydIcon name="calendar" size={20} color={c.ink} /><Text style={local.copy}>{copy('Un rendez-vous. Des visages familiers. Une bonne raison de ressortir.', 'A meetup. Familiar faces. A reason to head out again.')}</Text></View>}
        ...
        {section === 'life' ? <>
          ...
          <View style={local.memberList}>{filteredMembers.map(member => {
            ...
            const memberRole = crew.overview?.contributions.find(item => item.userId === member.userId)?.role;
            const roleLabel = memberRole && isCrewRole(memberRole) ? t(CREW_ROLE_E[memberRole]) : null;
            const identity = !hidden ? memberProfiles.data?.find(person => person.id === member.userId) : undefined;
            return <View key={member.userId} style={local.member}>{identity ? <SocialAvatar2026 person={identity} size={40} /> : <View style={local.memberAvatar}><Text style={local.memberInitial}>{hidden ? '·' : displayed.slice(0, 1).toLocaleUpperCase()}</Text></View>}<Pressable style={local.memberIdentity} accessibilityRole="button" disabled={hidden} aria-disabled={hidden} accessibilityState={{ disabled: hidden }} onPress={() => router.push({ pathname: '/member', params: { userId: member.userId } })}><Text style={local.rowTitle}>{identity?.name ?? displayed}{member.isMe ? copy(' · toi', ' · you') : ''}</Text><Text style={local.micro}>{hidden ? copy('Profil masqué', 'Hidden profile') : roleLabel ?? copy('Voir le profil', 'View profile')}</Text>{!hidden && isCrewSportingRole2026(sportingRole) ? <Text style={local.micro}>{sportingRoleLabel2026(sportingRole, locale === 'en')} · {copy('volontaire', 'voluntary')}</Text> : null}</Pressable>
              {!member.isMe ? <Pressable accessibilityRole="button" accessibilityLabel={copy(`Actions pour ${displayed}`, `Actions for ${displayed}`)} style={local.actionCircle} onPress={() => setSelectedMember(member)}><GrydIcon name="plus" size={20} color={c.ink} /></Pressable> : null}
            </View>;
          })}</View>
          ...
          <ProfileLink tone="light" title={copy('Administrer le crew', 'Manage the crew')} icon="reglages" onPress={() => router.push('/crew-edit')} />
          <ProfileLink tone="light" title={copy('Quitter ce crew', 'Leave this crew')} icon="fermer" onPress={() => {
            setError(null);
            if (leaveVerdict(crew.overview?.myRole ?? '', crew.memberCount) === 'must_transfer_lead') setError(copy('Transmets la direction à un autre membre avant de quitter ce crew. Les actions d’un membre permettent de le faire.', 'Transfer leadership to another member before leaving this crew. Open a member’s actions to do this.'));
    ...
    <PlayerModerationSheet pseudo={selectedMember?.pseudo ?? null} onClose={() => setSelectedMember(null)} crew={selectedMember ? { userId: selectedMember.userId, actorRole: crew.overview?.myRole ?? '', targetRole: crew.overview?.contributions.find(item => item.userId === selectedMember.userId)?.role ?? '', onChanged: crew.reload } : null} />
  </>;
}
...
const local = StyleSheet.create({
  ...
  state: { backgroundColor: c.surface, padding: 20, borderRadius: 24, gap: 14 }, form: { gap: 16, paddingTop: 8, paddingBottom: 20 }, back: { minHeight: 44, flexDirection: 'row', alignItems: 'center', gap: 6 }, input: { minHeight: 52, paddingHorizontal: 16, borderRadius: 16, backgroundColor: c.surface, borderWidth: 1, borderColor: c.border, fontFamily: fonts.text, fontSize: 15, color: c.ink }, code: { fontFamily: fonts.mono, fontSize: 20, letterSpacing: 4, textAlign: 'center' }, access: { minHeight: 82, backgroundColor: c.surface, borderRadius: 20, borderWidth: 1, borderColor: c.surface, padding: 16, flexDirection: 'row', alignItems: 'center', gap: 14 }, accessSelected: { borderColor: c.ink },
});
