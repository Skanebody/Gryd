...
export function captureExplanation2026(receipt: CaptureReceipt2026 | undefined, fr: boolean): { title: string; body: string } | null {
  ...
  if (receipt.reason === 'verification_required') return { title: t('Vérification nécessaire', 'Verification required'), body: t('Cette activité nécessite une vérification avant de modifier le terrain partagé. Ta sortie est enregistrée.', 'This activity needs verification before it changes shared terrain. Your activity is saved.') };
