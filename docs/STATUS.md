...
> **Le P0** : « je ne suis pas connecté, l'app s'ouvre sur la carte et je n'ai aucun moyen de créer mon compte ». Cause : la seule porte de la carte vivait dans la feuille Couches, `/sign-in` ne prononçait jamais « créer un compte », et une vingtaine d'écrans peignaient « Connexion » seul ou disparaissaient quand `configured` était faux. Corrigé par UNE grammaire (`features/account/AccountDoor2026.tsx`, familles `profile` et `ui`) : titre « Crée ton compte ou connecte-toi », CTA chartreuse « Créer mon compte », note « ou se connecter », état « Serveur non configuré sur ce build » au lieu de `null` ; montée sur la carte (bandeau fermable sous le logo + avis bas + Couches), le Profil invité, Crew, et 20 autres écrans ; bouton Apple en `CONTINUE` (« Continuer avec Apple ») sur les deux écrans ; `/sign-in` et `/email` disent que le lien crée le compte. Commits `f86220c`, `2cd902a`, `cb5a6ee`, `fe9d879`, `804a1c9` (+ lot 11 pour les 7 dernières portes : `qr`, `classement-commune`, `premium`, `historique`, `course/[id]`, `parametres/[section]` ×2). Verrous : `accountDoor2026.test.ts`, `accountDoorShared2026.test.ts` (33), `accountDoorButtons2026.test.ts` (21).
