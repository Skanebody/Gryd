...
## ADR-018 — 2026-09-11 — Quatre arbitrages du soir : notifications in-app d’abord, gestionnaire de crew tel que livré, cosmétiques répartis, tracés GPS jamais purgés sans le joueur

### Les décisions du fondateur, mot pour mot (11/09/2026, 22 h 50)
« mets en place un producteur de notification, les notification doivent etre courtes avec un emoji. Gestionnaire de crew, fais ce qui te semble le plus adapté. Il faut aussi un QR code de crew. Cosmétique, ce qui te semble le mieux adapté. Trace GPS ce qui est le plus adapté ou mettre dans les reglage l'option mais ne pas purger directement ».

### Décision
1. **Notifications : le canal in-app est construit en entier, la push reste en attente.** Un producteur serveur écrit une boîte de réception (`notification_inbox_2026`) pour chaque fait de jeu déjà mesuré (capture publiée, vérification, défi, crew, avertissement, retrait, dissolution, parrainage, quête, niveau), en passant par la seule porte qui connaît le budget de sollicitations et la plage calme (`claim_notification_2026`, 0141). Chaque texte est court, commence par un emoji, tutoie, sans tiret long, ne nomme jamais un tiers dans un retrait. L’app lit la boîte dans un centre de notifications et une cloche dont le compteur disparaît à zéro (jamais une pastille permanente, cahier §14). La push APNs n’est pas activée : aucun jeton collecté, aucune promesse à l’écran ; ce qu’elle exigera est écrit dans `docs/product/GRYD_NOTIFICATIONS_2026_09.md`. Elle reste la tension n° 1 d’ADR-013.
2. **Gestionnaire de crew : les cinq arbitrages de la spécification sont retenus tels que livrés** (`docs/product/GRYD_GESTION_CREW_2026_09.md` §5 et §6) : retrait automatique existant mais éteint par défaut, km visibles au capitaine uniquement pour les règles qu’il a armées, re-adhésion refusée 30 jours au même crew seulement, avertissement transactionnel (jamais de retrait sans avertissement lu ou vieux du délai de grâce), entraide non mesurée. S’y ajoutent le QR de crew (même jeton que le lien, révocable) et un scanner dans l’app (`expo-camera`, nouveau build), parce qu’iOS n’ouvre pas un schéma `gryd://` depuis son appareil photo tant que le domaine n’existe pas.
3. **Cosmétiques : pas de quatrième collection.** Les douze objets commerciaux restent répartis dans les trois collections du cahier §16.1 (contour, relief, clubhouse) ; les objets de parrainage forment leur propre origine, réservée, jamais vendue. Aucun prix n’est écrit.
4. **Tracés GPS : conservés par défaut, effacés seulement si le joueur le demande.** Une préférence `keep` (défaut) · 90 jours · 1 an, réglable dans Confidentialité et données, s’applique aux deux formes de la trace ; un job quotidien ne touche que les joueurs qui ont choisi une durée ; une action « Supprimer le tracé de cette sortie » efface immédiatement la trace d’une sortie sans toucher ses statistiques ni le terrain ; aucune trace n’est purgée tant qu’une revue anti-triche est ouverte. La purge aveugle de `polyline_masked` à 90 jours (0101) est remplacée par cette règle.
