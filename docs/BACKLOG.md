...
- **Écran de CONNEXION MVP.** Le groupe `(mvp)` n'en a aucun : la bascule d'entrée s'arrête donc à l'accueil, et `/sign-in` + `/onboarding` restent legacy. C'est le dernier morceau de parcours qui n'est pas reconstruit.
- **Drapeau d'onboarding MVP.** La garde lit `onboardingDone` depuis un hook legacy qu'ADR-001 interdit d'importer. Tant qu'un drapeau MVP ne le remplace pas, `/bienvenue` et `/position` ne sont atteints que par URL directe — et les lignes `KNOWN_ORPHANS` correspondantes doivent disparaître ce jour-là.
