...
1. `ownership_clusters` — les possessions réellement capturées (`ownership_2026`) dans la commune, groupées en trois par `ST_ClusterKMeans`, enveloppe convexe élargie du seuil de trace du cahier, découpée sur l’emprise communale **quand elle existe**. C’est la source à privilégier : les secteurs sont là où l’on court déjà. `p_search_radius_m` borne la **recherche** de possessions, il ne dessine jamais : sans contour administratif, aucun secteur n’est coupé sur le disque, sinon chacun hériterait d’un bord en arc de cercle inventé.
