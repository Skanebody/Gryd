...
- [x] Étape 3 · célébrer le PIONNIER de commune : hero « TU AS OUVERT {commune} · Personne n'avait couru ici avant toi » (nom réel serveur, remplace « TERRITOIRE ÉTENDU », copie sans genre 5 langues) + city_opened{source:'run'} + symétrie source:'manual' sur CityPicker ; helper pur gardé (jamais sans verdict, jamais sur refus §11, test 4/4) ; revue SHIP — S — `96299e9` (s'allume avec O1)
- [ ] Étape 3b · statut « Fondateur·rice de {commune} » PERSISTANT sur le Profil (reachable, sinon la célébration s'évapore) — DÉCISION FONDATEUR : identité publique/partageable ? local vs lecture serveur ? — exerçable seulement post-O1 — S
- [ ] Retry hors-ligne perd l'analytics de récompense : `retryPendingUpload` (pendingUpload.ts:110) ne capture même pas `data` sur renvoi RÉUSSI (`const { error } = ...`) → une capture/un pionnier faits SANS réseau (là où les communes sont vierges) resend plus tard, le serveur ouvre bien la commune, mais le client ne tire NI claim_result NI city_opened NI time_to_first_capture → funnel aveugle sur les captures hors-ligne. Fix décision-libre : traiter le corps de succès (fire les events, source:'pending_retry' ; PAS de replay UI, le moment est passé) + EXTRAIRE un helper partagé `emitRunResultAnalytics(result, source)` réutilisé par useRealRunCore.uploadOrQueue (sinon les deux chemins divergent). Pré-existant, pas un mensonge (aucun fantôme). Ne compte QUE post-O1 (pré-O1 aucun 'sent'). — M
- [x] Étape 4 · notif « RENDEZ-VOUS » 100% LOCALE (scheduleNotificationAsync, trigger DAILY, aucun token/APNs/FCM) : opt-in secondaire sur course-result, permission sur geste, web→rien, refus→message honnête, état « posé » RÉCONCILIÉ avec l'OS (jamais un rappel fantôme), copy anti-shame 5 langues ; revue SHIP — M — `a7c4ac2`. Décision fondateur : heure défaut (18h) + ton + futur choix d'heure joueur. Suites mineures : denied→openSettings, setNotificationHandler premier-plan, libellé 12/24h.
- [x] Étape 5 · SÉRIE LOCALE : journal local de timestamps (aucune position) → même moteur `computeStreak` → StreakBlock sur le résultat UNIQUEMENT sans verdict serveur (hors-ligne/pré-O1) ; l'autorité serveur prime toujours, ne peut jamais être contredite ; cœur pur testé 6/6 ; revue SHIP — S — `1ebbd31`. DIFFÉRÉ (étape 5b) : câbler l'écran Aujourd'hui (orphelin) dans la nav — il lit des RPC O1-gatées, à exposer APRÈS O1 en vérifiant qu'il dégrade honnêtement.
...
- [~] Row 5 · P1 — Classements §16 spécialités CODE-COMPLET : vue serveur specialty_leaderboard 0069 (PGlite 4/4) `de871fe` + UI client (useSpecialtyLeaderboard + onglet « Spécialités » + sélecteur Conquérant/Défenseur/Voleur/Pionnier, vide honnête, revue SHIP) `d26f930`. flags.season = simple visibilité (PAS un bloqueur). RESTE UNIQUEMENT : DÉPLOIEMENT prod de 0069 (fondateur : `npx supabase db push`) → les spécialités s'allument avec les vraies données. Échelle géographique (quartier/région/national) + crew board (matview morte) + amis = nouvelles vues serveur, chantiers séparés.
