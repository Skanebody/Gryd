...
- [~] Row 5 · P1 — Classements §16 spécialités : MOITIÉ SERVEUR faite (vue specialty_leaderboard 0069, exposant hexes_captured/steals/defends/pioneer cross-user par ville, RLS definer comme player_leaderboard, PGlite 4/4) — `de871fe`. flags.season = simple visibilité (PAS un bloqueur, tranché par l'audit). RESTE : (a) UI client (chip task_515f92bc : useSpecialtyLeaderboard + sélecteur spécialité, vide honnête) ; (b) DÉPLOIEMENT prod de 0069 (fondateur : npx supabase db push). Échelle géographique (quartier/région/national) + crew board + amis = nouvelles vues serveur, chantiers séparés.
