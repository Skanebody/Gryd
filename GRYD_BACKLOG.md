...
- [ ] Retry hors-ligne perd le result-reveal : `retryPendingUpload` (pendingUpload.ts:72-116) jette le corps sur renvoi réussi → un pionnier/une capture faits SANS réseau (là où les communes sont vierges) perdent hero + city_opened + claim_result. Limite architecturale préexistante, pas un mensonge — faire traiter le corps de succès par retryPendingUpload — M
