...
- [x] Retry hors-ligne perd l'analytics de récompense — FAIT `4e27724` : émetteur partagé `emitRunResultAnalytics(result, source)` (activation.ts) réutilisé par le chemin live ET le renvoi ; verrou `retryInFlight` anti-double-émission (revue FIX appliquée). Reste vigilance : threader `startedAt` dans ttfc pour un pionnier resynchronisé tard.
